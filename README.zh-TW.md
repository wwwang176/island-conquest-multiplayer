...
一款多人連線 3D 第一人稱射擊遊戲，完全在瀏覽器中運行。Node.js 伺服器運行權威遊戲模擬 — 兩支 AI 隊伍各 15 名士兵，編成 5 個 3 人小隊，在程序化生成的熱帶島嶼上爭奪 5 個旗點。多名玩家可透過 WebSocket 連線，加入任一隊伍親自參戰，也可以即時旁觀戰鬥。
...
在瀏覽器中開啟 `http://localhost:8088`。同一區域網路的其他玩家可使用主機的區網 IP 連線（例如 `http://192.168.1.x:8088`）。

## 技術棧

- **Node.js** — 權威遊戲伺服器
- **ws** — WebSocket 伺服器，即時二進位協定
...
1. 開啟遊戲網址，輸入伺服器位址（同一主機會自動偵測）。
2. 按 **Connect** 連線 — 進入觀戰模式。
3. 按 **1**（A 隊）或 **2**（B 隊）。
4. 出現武器選擇畫面 — 用 **1–4** 選擇武器。
5. 按 **SPACE** 部署。游標會被鎖定進入 FPS 操控。
6. 死亡後有 5 秒等待時間，之後可選擇新武器並按 **SPACE** 重生。
7. 按 **ESC** 兩次（第一次解除游標鎖定，第二次確認）離隊回到觀戰模式。
...
每隊各有一架直升機在基地生成。可搭載 1 名駕駛員 + 4 名乘客。

- **血量**：12,000
- **最大速度**：水平 45 m/s、垂直 8 m/s
- **登機半徑**：3.6m — 按 **E** 登機，再按 **E** 離開
- **駕駛員**控制飛行（WASD + SPACE/SHIFT）；乘客可從座位射擊
- 被擊毀的直升機會以物理模擬墜毀
...
連續性系統（瞄準、射擊、移動）每幀為全部 30 名士兵執行；行為樹和威脅掃描則每 tick 分批 8 人執行以維持效能。
...
- **反應延遲**：基礎值 150–250ms（依個性），由四個因子動態調整：
  - **距離**：在 0–60m 間線性插值 `nearReaction` 到 `farReaction`。突擊兵近距離反應極快（×0.3）但遠距離遲鈍（×1.4）；狙擊手相反（近 ×1.4、遠 ×0.5）
  - **瞄準角度**：敵人越靠近準星中心反應越快（×0.93）；在視野邊緣則更慢（最高 ×1.33）
  - **視線品質**：僅露頭目標（部分掩體）增加 40% 延遲（×1.4）
  - **環境**：暴風/夜間倍率（預設 ×1.0）
...
- 基於高度的頂點著色（沙灘 → 草地 → 岩石）
- 植被：棕櫚樹（含地面投影陰影）
- 掩體物件：岩石、沙袋、木箱、牆壁、防禦工事

---

## 架構

遊戲使用**客戶端-伺服器架構**。伺服器以每秒 64 tick 運行權威模擬（物理、AI、命中判定、旗點佔領），並透過 WebSocket 將二進位快照串流至所有連線的客戶端。客戶端負責渲染世界、插值遠端實體、預測本地玩家移動。

```
server.js                 伺服器入口 — 建立 ServerGame + NetworkManager
src/
  client-main.js          客戶端入口 — 建立 ClientGame 實例
  client/
    ClientGame.js          客戶端協調器：渲染器、場景、攝影機、HUD、輸入
    NetworkClient.js       WebSocket 連線 + 二進位訊息處理
    EntityRenderer.js      遠端實體插值渲染
    Interpolation.js       快照插值邏輯
    FPSController.js       第一人稱攝影機 + 客戶端移動預測
    VehicleController.js   載具攝影機 + 乘員定位
    VehicleRenderer.js     直升機 3D 模型 + 動畫
    SpectatorController.js 觀戰攝影機模式（跟隨 / 鳥瞰）
    ClientHUD.js           DOM 疊加層：準心、彈藥、血量、旗點
    JoinScreen.js          連線表單 + 隊伍/武器選擇 UI
    DeathScreen.js         死亡 / 重生畫面
    GameOverScreen.js      回合結束畫面
    Scoreboard.js          TAB 計分板疊加層
    WeaponCardUI.js        武器選擇卡片
  server/
    ServerGame.js          權威遊戲循環 + 模擬
    NetworkManager.js      HTTP 靜態伺服器 + WebSocket 廣播
    ServerAIManager.js     伺服器端 AI 管理器（建立與更新所有 AI）
    ServerSoldier.js       伺服器端士兵實體（物理 + HP）
    ServerPlayer.js        伺服器端玩家實體（輸入處理）
    ServerPhysics.js       cannon-es 物理世界封裝
    ServerIsland.js        伺服器端地形 + 碰撞體生成
    ServerHelicopter.js    伺服器端直升機模擬
    ServerVehicleManager.js 載具生命週期管理
    ServerGrenadeManager.js 手榴彈生成 + 爆炸邏輯
    RateLimiter.js         每客戶端速率限制
  shared/
    constants.js           Tick rate、連接埠、地圖大小、遊戲常數
    protocol.js            二進位訊息類型 + 編碼/解碼輔助函數
    DamageModel.js         共用傷害計算
    DamageFalloff.js       距離衰減曲線
    CapsuleBody.js         膠囊物理體輔助
  core/
    EventBus.js            發布/訂閱訊息系統
    InputManager.js        鍵盤/滑鼠輸入 + 指標鎖定
  entities/
    WeaponDefs.js          武器數值定義
    Grenade.js             手榴彈實體
    Vehicle.js             載具基底類別
    Helicopter.js          直升機飛行模型 + 座位
  world/
    Island.js              程序化地形 + 植被
    CoverSystem.js         掩體點登錄
    FlagPoint.js           旗點佔領機制
    Fortification.js       防禦工事生成
    Noise.js               Simplex 噪音
  ai/
    AIController.js        每士兵行為樹 + 戰鬥
    AIMovement.js          AI 移動與路徑跟隨
    AIShooter.js           AI 瞄準與射擊邏輯
    AIGrenade.js           AI 手榴彈使用邏輯
    AIVisual.js            AI 視線掃描
    AIVehicleController.js AI 直升機駕駛
  ...
  workers/
    navgrid-worker.js      背景執行緒 NavGrid 建構（瀏覽器）
    navgrid-worker-node.js 背景執行緒 NavGrid 建構（Node.js）
    pathfind-worker.js     背景執行緒 A* 尋路（瀏覽器）
    pathfind-worker-node.js 背景執行緒 A* 尋路（Node.js）
    threat-worker.js       背景執行緒威脅地圖計算（瀏覽器）
    threat-worker-node.js  背景執行緒威脅地圖計算（Node.js）
    threat-scan-worker.js  背景執行緒威脅掃描（瀏覽器）
    threat-scan-worker-node.js 背景執行緒威脅掃描（Node.js）
```
